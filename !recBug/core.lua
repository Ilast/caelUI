...
----------------------------------------
local font = CreateFont("neuropol")
font:SetFont([[Interface\Addons\caelMedia\Fonts\neuropol x cd rg.ttf]], 11, nil)
...
local function make_button(name, text)
	...
	button:SetHeight(28)
	button:SetWidth(28)
	button:SetNormalFontObject(neuropol)
	button:SetText(text)
...
output_frame:SetFontObject(neuropol)
...
local function show_error(index)
	if errors[index] then
		...
		if not error_frame:IsShown() then
			error_frame:Show()
		end
	end
end
