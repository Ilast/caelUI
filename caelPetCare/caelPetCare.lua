...
caelPetCare = CreateFrame("Frame")
caelPetCare:SetScript("OnEvent", function(self, event, ...) if self[event] then return self[event](self, ...) end end)
caelPetCare:RegisterEvent("ADDON_LOADED")
...
function caelPetCare:ADDON_LOADED(addon)
	...
	self.db = {
		...
		["useBonus"] = false, -- Include in your pet's diet foods which provide stat bonuses when eaten
		["useRaw"] = false, -- Include in your pet's diet foods which may be used by the Cooking profession
		["showFood"] = true, -- Show the icon of the food item that will be consumed, instead of the Feed Pet spell icon
		["mendThreshold"] = 0.75, -- Mend your pet out of combat by default when its health is below this percent
		["useConjured"] = true, -- Include in your pet's diet foods conjured by mages
		["mendModifier"] = "shift", -- Use this modifier key to mend your pet out of combat, or revive it in combat if it's despawned
		["warnHungry"] = true, -- Print a notice when your pet is less than happy
		["useCombo"] = true, -- Include in your pet's diet foods which restore both health and mana when eaten
		["feedHappy"] = false, -- Automatically consume food items even if your pet is already happy
		["dismissModifier"] = "ctrl", -- Use this modifier key to dismiss your pet
	}

	if caelPetCareStrings then caelPetCareStrings = nil end
...
function caelPetCare:PLAYER_LOGIN(delayed)
...
function caelPetCare:BAG_UPDATE()
...
function caelPetCare:CHARACTER_POINTS_CHANGED()

	improved = select(5, GetTalentInfo(1, 10)) > 0

end

caelPetCare.PLAYER_TALENT_UPDATE = caelPetCare.CHARACTER_POINTS_CHANGED

------------------------------------------------------------------------

function caelPetCare:PLAYER_ALIVE()
	if UnitIsGhost("player") then return end

	self:Edit()
end

caelPetCare.PLAYER_UNGHOST = caelPetCare.PLAYER_ALIVE

------------------------------------------------------------------------

function caelPetCare:PLAYER_REGEN_DISABLED()
...
function caelPetCare:PLAYER_REGEN_ENABLED()
...
function caelPetCare:UI_ERROR_MESSAGE(message)
...
function caelPetCare:UNIT_AURA(unit)
...
function caelPetCare:UNIT_HAPPINESS(unit)
	...
	if self.db.warnHungry then
		if happiness == 1 then
			if (GetTime() - warned) > 60 then
				print(format("|cffD7BEA5cael|rPetCare: ".."%s is very hungry!", UnitName("pet")))
				warned = GetTime()
			end
		elseif happiness == 2 then
			if (GetTime() - warned) > 120 then
				print(format("|cffD7BEA5cael|rPetCare: ".."%s is hungry.", UnitName("pet")))
				warned = GetTime()
			end
		end
	end
...
function caelPetCare:UNIT_HEALTH(unit)
	if unit ~= "pet" then return end
...
function caelPetCare:UNIT_PET(unit)
...
function caelPetCare:Scan()
	...
	if petlvl and petlvl > 0 then
		...
		if not best.id and UnitName("pet") ~= "Unknown" and self.db.warnFood and GetTime() - warned > 240 then
			print(format("|cffD7BEA5cael|rPetCare: ".."You don't have any food for %s.", UnitName("pet")))
			warned = GetTime()
		end
		self:Edit()
	end
